...
```{r pcas, cache=TRUE, fig.width=7, fig.height=6}
...
```
**Look at variable importance plots as another way to compare the two models.  First, the model based on original data.**  
```{r varImpPlot1, fig.width=7, fig.height=6}
varImpPlot(rf.k5.503$finalModel)
```

**Second, the model based on extreme values replaced with medians.**
```{r varImpPlot2, fig.width=7, fig.height=6}
varImpPlot(rf.k5.OLs.503$finalModel)
```

**There was no substantial difference between the value metrics of the two models' predictions.  Because of that, along with the fact that 10 out of the 11 outlying values came from a single observation/subject [and thus were unlikley to represent a true association between an extreme field measurement and a particular criterion level(s)], we decided to make our test-set predictions with the random forest model fitted to the version of the training set with median-replaced extreme values.  Also, the variable importance plot for that set/model showed a smoother distribution curve.**

**Fit the model using the out-of-bag error estimate as the measure of accuracy.**
```{r rf.OLs.503.OOB, cache=TRUE}
rf.OLs.503.OOB=train(classe~., data=trnOLs, method="rf", trControl=trainControl(method="oob"), ntree=503)
rf.OLs.503.OOB
```

**Note that in the results directly above, accuracy based on the out-of-bag error estimate was 1.00.  Obviously, we did not base our expectation of out-of-bag error on that datum.  Rather, we based it on the output from the model fit using 5-fold cross-validation.  Given the 95% confidence interval for accuracy for that model, we expect to have an out-of-sample error rate of 0.3 - 0.7%.  Given the use of a random forest model with cross-validation and given the simplicity of the analyses, we do not believe that overfitting occurred and thus do not believe that there is any reason to increase our expected error rate beyond that indicated by the accuracy confidence interval.**    
